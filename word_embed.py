...
def comb_vocab_dict(feat_name, embed_size, data_dict, test_mode, data_fold_path, save_fold_path, use_saved_word_feats, save_word_feats,filename):
	voc_filename = "%scomb_vocab~%s~%s.pickle" % (save_fold_path, feat_name, filename)
	if use_saved_word_feats and os.path.isfile(voc_filename):
		...
		filename = "%scomb_word_list~%s.pickle" % (save_fold_path,filename)
...
def word_featurize(word_feats_raw, model_type, data_dict, poss_word_feats_emb_dict, use_saved_word_feats, save_word_feats, data_fold_path, save_fold_path, test_mode,filename):
	...
	for word_feat_raw_dict in word_feats_raw:
		...
		if 'tune' in word_feat_raw_dict and word_feat_raw_dict['tune'] == True:
			var_tune = True
			di_filename = ("%sad_word_feat~%s~%s~%s~%s~%s.pickle" % (save_fold_path, word_feat_name, var_model_hier, var_tune, test_mode,filename))
		else:
			var_tune = False
			di_filename = ("%sad_word_feat~%s~%s~%s~%s.pickle" % (save_fold_path, word_feat_name, var_model_hier, var_tune,filename))

		comb_str = ("%s~%s~%s~%s" % (word_feat_name, var_model_hier, var_tune,filename))
		...
		if use_saved_word_feats and os.path.isfile(di_filename):
			...
			if word_feat_name == 'glove' or word_feat_name == 'fasttext':
				comb_vocab = comb_vocab_dict(word_feat_name, poss_word_feats_emb_dict[word_feat_name], data_dict, test_mode, data_fold_path, save_fold_path, use_saved_word_feats, save_word_feats,filename)
				if var_tune == True:
					ad_word_feats['filepath'] = save_fold_path + 'word_vecs~' + word_feat_name + '~' + filename + '/' + str(var_model_hier) + '/' + str(var_tune) + '/' + str(test_mode) + '/'
					...
					ad_word_feats['filepath'] = save_fold_path + 'word_vecs~' + word_feat_name + '~' + filename+ '/' + str(var_model_hier) + '/' + str(var_tune) + '/'
					os.makedirs(ad_word_feats['filepath'], exist_ok=True)
				...
				ad_word_feats['filepath'] = save_fold_path + 'word_vecs~' + word_feat_name + '~' + filename + '/' + str(var_tune) + '/'
